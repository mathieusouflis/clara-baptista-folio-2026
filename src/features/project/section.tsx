...
export const ProjectSection = (props: { content: NonNullable<Project["content"]> }) => {
  ...
  const contentImages = content.images && typeof content.images === "object" ? content.images.filter(i => typeof i !== "number") : []

  const [currentImage, setCurrentImage] = useState(contentImages[0] ? contentImages[0] : undefined)

  return <Grid key={content.id} as="section" id={content.id ?? ""} >
  <GridItem
    span={"full"}
