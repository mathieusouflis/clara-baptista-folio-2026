...
export async function ProjectPage({ categoryId, projectId }: { categoryId: number, projectId: number }) {
...
console.log(projects)
  ...
  return <Grid>
    ...
    <GridItem start={5} end={9} className='flex flex-col justify-center items-center h-screen py-(--grid-margin) gap-7'>
      <span className="relative h-1/2 max-h-1/2 flex items-center justify-center">
        <Link href={`/categories/${categoryId}/${projects.hasPrevPage ? projects.prevPage : projects.totalPages}`}>
      ...
      <div className='w-full px-9 py-2.5 rounded-full flex flex-row items-center gap-6 bg-[#070092] min-h-28'>
        ...
        <div className="flex flex-col w-full text-white">
          ...
          {project.releaseDate && <p className='opacity-60 text-[14px]'>{months[new Date(project.releaseDate).getMonth()]} {new Date(project.releaseDate).getFullYear()}</p>}
        </div>
      </div>
    </GridItem>
